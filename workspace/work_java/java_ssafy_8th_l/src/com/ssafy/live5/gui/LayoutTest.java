...
public class LayoutTest extends JFrame {
	...
	private void launchUi() {
		...
		makeProblemPanel();
		// Frame 기본 설정
		this.setTitle("layout test"); // ecplise name
		this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // x누르면 죽어라
		this.pack();
		this.setVisible(true); // default가 인비져블
	}
			...
			public void keyReleased(KeyEvent e) {
				if (e.getKeyCode() == 10) {
					...
					if (val == answer) {
						...
						lWrongCnt.setText(pre + 1 + ""); // setText는 문자열만 가
					}
					makeProblem();
				}
